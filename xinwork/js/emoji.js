...
(function(a) {
	var b = (function() {
		...
		c.createSpan = function(e) {
			...
			f.setAttribute("style", "height: " + this.size + "px; width: " + this.size + "px; display: inline-block; font-size: 20px !important; text-align: center; line-height: 24px;");
		...
		c.symbolToEmoji = function(e) {
			...
			return e.replace(/\[.+?\]/g, function(g) {
				var f = g.slice(1, g.length - 1);
				if(d.regExpName.test(f)) {
					return f.replace(d.regExpName, function(i) {
						for(var h in d.emojiFactory) {
							if(d.emojiFactory[h].zh == i) {
								return d.emojiFactory[h].tag
							}
						}
					})
				} else {
		...
		c.size = 24;
		c.url = window.SCHEMETYPE ? window.SCHEMETYPE + "://cdn.ronghub.com/css-sprite_bg-2.1.10.png" : "//cdn.ronghub.com/css-sprite_bg-2.1.10.png";
